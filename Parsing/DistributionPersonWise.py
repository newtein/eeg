...
alcoholicData = {}
controlGroupData = {}
graphDir = makeDir("./PersonWiseDistribution")
...
for person in persons:
	...
	if(person[3] == 'a'):
		alcoholicFlag = True
		personDir = makeDir(alcoholDir + "/" + person)
	else:
		alcoholicFlag = False
		personDir = makeDir(controlDir + "/" + person)
	...
	for file in files:
		...
		try:
			...
			if(text.find("err") != -1):
				...
				print("Here")
				continue
			typeDir = makeDir(personDir + "/" + typeOfTest)
		except:
			continue
		...
		try:
			...
			trialNumber = 'Unknown'
		trialDir = makeDir(typeDir + "/" + str(trialNumber))
		channel = ''
		channel_data = []
